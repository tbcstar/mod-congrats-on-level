    ...
    void OnLogin(Player* player)
    {
        ...
        if (sConfigMgr->GetOption<bool>("Congrats.Announce", true))
        {
            ...
            switch (player->GetSession()->GetSessionDbLocaleIndex())
            {
                ...
                case LOCALE_ruRU:
                {
                    messageModule = "此服务器正在运行 |cff4CFF00升级奖励 |r模块。";
                    break;
                }
                case LOCALE_esES:
                case LOCALE_esMX:
                {
                    messageModule = "此服务器正在运行模块|cff4CFF00升级奖励|r";
                }
                default:
                    break;
            }
        ...
        if (sConfigMgr->GetOption<bool>("Congrats.Enable", true)) {
            ...
            if (!level.empty())
            {
                ...
                switch (player->GetSession()->GetSessionDbLocaleIndex())
                {
                    ...
                    case LOCALE_ruRU:
                    {
                        ss2 << "恭喜升级 " << level << " " << player->GetName() << "! 你被授予了 " << money << " 黄金和一些宝物！";
                        break;
                    }
                    case LOCALE_esES:
                    case LOCALE_esMX:
                    {
                        ss2 << "祝贺你的等级 " << level << " " << player->GetName() << " 已被授予 " << money << " 黄金和一些宝物！";
                    }
                    default:
                        break;
                }
